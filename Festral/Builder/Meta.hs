...
instance Show Meta where
    show (Meta board bType commit bTime toolc builder stat hash _ repName branch) = "BOARD=" ++ board
                                                    ...
                                                    ++ "\nREPO_NAME=" ++ repName
                                                    ++ "\nBRANCH=" ++ branch
    show (MetaTest m t tName tTime testName) = (show m)
                                        ++ "\nTESTER=" ++ t
                                        ++ "\nTESTER_NAME=" ++ tName
                                        ++ "\nTEST_TIME=" ++ tTime
                                        ++ "\nTEST_NAME=" ++ testName
...
readMeta str = readMeta' (f str)
    where
    readMeta' (board:bType:commit:bTime:toolc:builder:stat:hash:repName:branch:t:tName:tTime:testName:_) = MetaTest (Meta board bType commit bTime toolc builder stat hash "" repName branch) t tName tTime testName
