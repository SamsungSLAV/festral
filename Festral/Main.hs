...
runCmd (Options True _ _ _ _ _ _) = show <$> curlJobs >>= putStrLn

runCmd (Options _ (-1) _ ("") True _ _) = runCmd None

runCmd (Options _ (-1) _ f True _ _) = show <$> startJob f >>= putStrLn

runCmd (Options _ (-1) _ _ _ _ _) = putStrLn "Missing id of the quered job. Use -i option for set it."

runCmd (Options _ id _ ("") _ _ True) = show <$> getFileList id >>= putStrLn

runCmd (Options _ id _ fname _ _ True) = getJobOutFile id fname >>= justPutStrLn "No such job."

runCmd (Options _ id _ _ _ True _) = getJobOut id >>= putStrLn

runCmd (Options _ id True _ _ _ _) = show <$> getJobWhenDone id >>= putStrLn

runCmd (Options _ id False _ _ _ _) = show <$> getJob id >>= putStrLn
