...
Available commands are: `build`, `test`, `report`, `weles`, `server`, `check`.
...
After running this command program will do:
  ...
  5. create directory named as `commitSha1Hash_Bbuildtime` in the
...
`festral test` is used for runnig tests on Weles from existing builds and
...
other feathures. For put in your file HTML table with build results insert
...
The `farmer` is a tool for connecting to devices registered in the
...
sharing files with remote Weles server or just show logs and reports in
the web browser.
